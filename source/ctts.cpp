...
CTTSBOX::CTTSBOX(unsigned __int64 boxSize_, unsigned __int64 read_bytes_)
{
...
int CTTSBOX::anlysis()
{
	...
	mp4Info.trackInfor[mp4Info.current_track_ID].ctts_entry_count = entry_count;

	mp4Info.trackInfor[mp4Info.current_track_ID].ctts = (MP4CTTS*)malloc(entry_count*sizeof(MP4CTTS));
	if (!mp4Info.trackInfor[mp4Info.current_track_ID].ctts)
	{
		fprintf(g_mp4_log, "错误:ctts 申请内存出错 error %d\n", g_errors); g_errors++;
		return -1;
	}

	fprintf(g_mp4_log, "entry num     entry_count媒体片段个数     sample_offset每个片段的播放时间戳差\n");
	...
	for (unsigned int entry = 0; entry < entry_count; entry++)
	{
		...
		sample_offset = (((unsigned __int64)data[0]) << 24) + (((unsigned __int64)data[1]) << 16) + (((unsigned __int64)data[2]) << 8) + (((unsigned __int64)data[3]) << 0);
		sum_duration += (double)(sample_count*sample_offset);
		sum_sample_count += sample_count;
		read_bytes += 8;
		fprintf(g_mp4_log, "    %5d             %5d                 %5d（%.5lf秒）\n", entry, sample_count, sample_offset, (double)sample_offset / (double)mp4Info.trackInfor[mp4Info.current_track_ID].timescale);
		mp4Info.trackInfor[mp4Info.current_track_ID].ctts[entry].sample_count = sample_count;
		mp4Info.trackInfor[mp4Info.current_track_ID].ctts[entry].sample_offset = sample_offset;
	}
